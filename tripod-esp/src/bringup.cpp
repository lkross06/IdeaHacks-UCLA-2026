#include <Arduino.h>

// --- Pin Definitions ---
const int pinPWMA = 32;
const int pinAIN1 = 25;
const int pinAIN2 = 26;
const int pinPWMB = 33;
const int pinBIN1 = 27;
const int pinBIN2 = 14;
const int pinSTBY = 15;

// --- PWM Settings ---
const int freq = 500;      
const int resolution = 8;
const int chanA = 0;
const int chanB = 1;

// --- Timing Variables ---
unsigned long startTime;          
const unsigned long duration = 3000; 

void setMotor(char motor, int speed) {
    bool reverse = speed < 0;
    int absSpeed = abs(speed);

    if (motor == 'A') {
        digitalWrite(pinAIN1, !reverse);
        digitalWrite(pinAIN2, reverse);
        ledcWrite(chanA, absSpeed);
    } else {
        digitalWrite(pinBIN1, !reverse);
        digitalWrite(pinBIN2, reverse);
        ledcWrite(chanB, absSpeed);
    }
}

void setup() {
    Serial.begin(115200);

    pinMode(pinAIN1, OUTPUT);
    pinMode(pinAIN2, OUTPUT);
    pinMode(pinBIN1, OUTPUT);
    pinMode(pinBIN2, OUTPUT);
    pinMode(pinSTBY, OUTPUT);

    ledcSetup(chanA, freq, resolution);
    ledcSetup(chanB, freq, resolution);
    ledcAttachPin(pinPWMA, chanA);
    ledcAttachPin(pinPWMB, chanB);

    digitalWrite(pinSTBY, HIGH);
    Serial.println("System Ready - Tiered Speed Profile");

    for (int i =0; i < 8; i++){
        delay(1000);
        Serial.println(i);
        
    }

    // Start-time captured at the very end of setup()
    startTime = millis(); 
}

void loop() {
    unsigned long elapsedTime = millis() - startTime;

    if (elapsedTime < duration) {
        int currentSpeed;

        // --- Speed Plateau Logic ---
        if (elapsedTime < 500) {
            currentSpeed = 128;   // 30%
        } 
        else if (elapsedTime < 1000) {
            currentSpeed = 200;  // 65%
        } 
        else if (elapsedTime < 2000) {
            currentSpeed = 255;  // 100%
        } 
        else if (elapsedTime < 2500) {
            currentSpeed = 200;  // 65%
        } 
        else {
            currentSpeed = 128;   // 30%
        }

        // Apply negative for "opposite direction"
        int rampSpeed = currentSpeed * -1;

        // Step 1: A- B-
        setMotor('A', rampSpeed);
        setMotor('B', rampSpeed);
        delay(2); 

        // Step 2: A+ B-
        setMotor('A', abs(rampSpeed)); 
        setMotor('B', rampSpeed);      
        delay(2);

        // Step 3: A+ B+
        setMotor('A', abs(rampSpeed));
        setMotor('B', abs(rampSpeed));
        delay(2);

        // Step 4: A- B+
        setMotor('A', rampSpeed);
        setMotor('B', abs(rampSpeed));
        delay(2);
        
    } else {
        // Kill power and hit Standby
        setMotor('A', 0);
        setMotor('B', 0);
        digitalWrite(pinSTBY, LOW); 
        
        static bool stopped = false;
        if (!stopped) {
            Serial.println("\nProfile Complete. Standby Active.");
            stopped = true;
        }
    }
}